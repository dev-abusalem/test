...
function PageLink({ children, page }) {
  return (
    <article key={page.href}>
      ...
      >
        ...
        <Link
          ...
          aria-label={`Weiterlesen: ${page.title}`}
        >
           {children}
...
export function PageLinks({ title, pages, intro, readMoreText, className }) {
  return (
        ...
        <FadeInStagger className="grid grid-cols-1 gap-x-8 gap-y-16 lg:grid-cols-2">
          {pages.map((page) => (
            <FadeIn key={page.href}>
                <PageLink page={page}>{readMoreText}</PageLink>
            </FadeIn>
          ))}
        </FadeInStagger>
      </Container>
